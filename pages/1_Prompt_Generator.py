...
import os
from ui.layout import apply_dark_theme, app_header, sidebar_panel, get_or_set_api_key, set_api_key
...
apply_dark_theme()
app_header(
    title="Prompt Generator with State Management",
    caption="AI that gathers requirements and generates custom prompts",
    icon="🎯"
)
...
# Initialize API key with persistence
current_key = get_or_set_api_key("openai_key", "OPENAI_API_KEY", 
                                  validate_func=lambda k: k.startswith("sk-") if k else False)
if current_key and not st.session_state.get("openai_key"):
    st.session_state.openai_key = current_key
...
if not st.session_state.openai_key:
    ...
    if st.button("Connect", use_container_width=True):
        if api_key and api_key.startswith("sk-"):
            set_api_key("openai_key", api_key, "OPENAI_API_KEY")
            st.session_state.openai_key = api_key
            st.success("✅ Connected! Key saved for this session.")
            st.rerun()
        else:
...
for idx, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        # Display message content directly - fully visible
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        with loading_placeholder.container():
            st.markdown("🤔 **Thinking and processing...**")
            st.spinner("")
        ...
        # Clear loading and display response directly in chat bubble
        loading_placeholder.empty()
        # Display response as markdown - fully visible by default
